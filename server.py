...
app = FastAPI(title="Bhoomi RTC and MR Automation API")
...
def select_dropdown_by_text_contains(page, index, wanted_text):
    ...
    if not selected:
        ...
        raise Exception(f"Could not select '{wanted_text}' in dropdown {index}. Options: {options}")
...
def click_fetch_details(page):
    ...
    if not clicked:
        raise Exception("Fetch Details button not found")
...
def save_screenshot_as_pdf(browser, source_page, pdf_path):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            preview_btn = page.locator(
                'input[value="Preview"], button:has-text("Preview"), a:has-text("Preview")'
            ).first
            ...
            save_screenshot_as_pdf(browser, preview_page, pdf_path)
